...
void BitsetView::unite(const BitsetView &other) {
    assert(num_bits == other.num_bits);
    for (int i = 0; i < data.size(); ++i) {
        data[i] |= other.data[i];
    }
...
vector<BitsetMath::Block> PerStateBitset::get_duplicate(
    const State &state) {
    int num_blocks = BitsetMath::compute_num_blocks(num_bits_per_entry);
    vector<BitsetMath::Block> packed_bits(num_blocks, 0);
    BitsetView duplicate(
        ArrayView<BitsetMath::Block>(packed_bits.data(), num_blocks),
        num_bits_per_entry);
    BitsetView original = BitsetView(data[state], num_bits_per_entry);
    duplicate.unite(original);
    return packed_bits;
}
