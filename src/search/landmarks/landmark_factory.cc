...
shared_ptr<LandmarkGraph> LandmarkFactory::compute_lm_graph(
    const shared_ptr<AbstractTask> &task, bool _lama) {
    if (lm_graph) {
    ...
    if (from_file) {
        ...
        */
        ...
        for (auto &node_p : lm_graph->get_nodes()) {
            ...
            if (lama && node_p->is_true_in_state(initial_state))
                return;
...
void LandmarkFactory::edge_add(LandmarkNode &from, LandmarkNode &to,
    ...
    */
    ...
    if (acyclic) {
        if (type == EdgeType::REASONABLE ||
            type == EdgeType::OBEDIENT_REASONABLE) { // simple cycle test
...
void _add_options_to_parser(OptionParser &parser) {
    parser.add_option<bool>("reasonable_orders",
                            ...
                            "false");
    parser.add_option<bool>("obedient_reasonable_orders",
                            "generate obedient-reasonable orders",
                            "false");
