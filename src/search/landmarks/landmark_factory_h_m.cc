...
void LandmarkFactoryHM::print_pm_op(const VariablesProxy &variables, const PMOp &op) {
    ...
    for (size_t i = 0; i < op.cond_noops.size(); ++i) {
        ...
        for (; j < op.cond_noops[i].size(); ++j) {
            int fluent = op.cond_noops[i][j];

            print_fluentset(variables, h_m_table_[fluent].fluents);
            utils::g_log << endl;

            for (size_t k = 0; k < h_m_table_[fluent].fluents.size(); ++k) {
                cond_eff.insert(h_m_table_[fluent].fluents[k]);
            }
        }
        ...
        for (int set_index : all_lms) {
            ...
            if (use_orders) {
                for (int gn : h_m_table_[set_index].necessary) {
                    edge_add(*lm_node_table_[gn], *lm_node_table_[set_index],
