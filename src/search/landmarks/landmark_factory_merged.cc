...
void LandmarkFactoryMerged::generate_landmarks(
    ...
    // TODO: reimplement *compute* and *recompute*.
    if (first) {
        first = false;
        ...
        const State &state = TaskProxy(*task).get_initial_state();
        for (const shared_ptr<LandmarkFactory> &lm_factory : lm_factories) {
            lm_graphs.push_back(lm_factory->recompute_lm_graph(state));
        }
    }
            ...
            if (landmark.disjunctive) {
                /*
                  TODO: It seems that disjunctive landmarks are only added if
                   ...
                   landmark. This should either be more general (add only if
                   none of its subset is already there) or it should be done
                   only upon request (e.g., heuristics that consider orders
                   might want to keep all landmarks).
                */
                bool exists =
