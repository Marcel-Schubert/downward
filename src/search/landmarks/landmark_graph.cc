...
void LandmarkNode::finalize() {
    ...
    vector<LandmarkNode*> natural;
    vector<LandmarkNode*> reasonable;
    vector<LandmarkNode*> obedient_reasonable;
    for (const pair<LandmarkNode *, EdgeType> parent : parents) {
        switch (parent.second) {
            case EdgeType::NECESSARY:
                final_parents.push_back(parent.first);
                break;
            case EdgeType::GREEDY_NECESSARY:
                greedy_necessary.push_back(parent.first);
                break;
            case EdgeType::NATURAL:
                natural.push_back(parent.first);
                break;
            case EdgeType::REASONABLE:
                reasonable.push_back(parent.first);
                break;
            case EdgeType::OBEDIENT_REASONABLE:
                obedient_reasonable.push_back(parent.first);
                break;
        }
    }
    // first_greedy_necessary_parent = final_parents.size();
    final_parents.insert(final_parents.end(), greedy_necessary.begin(), greedy_necessary.end());
    first_natural_parent = final_parents.size();
    final_parents.insert(final_parents.end(), natural.begin(), natural.end());
    // first_reasonable_parent = final_parents.size();
    final_parents.insert(final_parents.end(), reasonable.begin(), reasonable.end());
    // first_reasonable_obedient_parent = final_parents.size();
    final_parents.insert(final_parents.end(), obedient_reasonable.begin(), obedient_reasonable.end());
    parents.clear();

    final_children.reserve(children.size());
    greedy_necessary.clear();
    natural.clear();
    reasonable.clear();
    obedient_reasonable.clear();

    for (const pair<LandmarkNode *, EdgeType> child : children) {
        switch (child.second) {
            case EdgeType::NECESSARY:
                final_children.push_back(child.first);
                break;
            case EdgeType::GREEDY_NECESSARY:
                greedy_necessary.push_back(child.first);
                break;
            case EdgeType::NATURAL:
                natural.push_back(child.first);
                break;
            case EdgeType::REASONABLE:
                reasonable.push_back(child.first);
                break;
            case EdgeType::OBEDIENT_REASONABLE:
                obedient_reasonable.push_back(child.first);
                break;
        }
    }

    // first_greedy_necessary_child = final_children.size();
    final_children.insert(final_children.end(), greedy_necessary.begin(), greedy_necessary.end());
    first_natural_child = final_children.size();
    final_children.insert(final_children.end(), natural.begin(), natural.end());
    first_reasonable_child = final_children.size();
    final_children.insert(final_children.end(), reasonable.begin(), reasonable.end());
    first_reasonable_obedient_child = final_children.size();
    final_children.insert(final_children.end(), obedient_reasonable.begin(), obedient_reasonable.end());
    children.clear();
}
...
void LandmarkGraph::finalize() {
    for (unique_ptr<LandmarkNode> &node : nodes) {
        node->finalize();
    }
}
}
