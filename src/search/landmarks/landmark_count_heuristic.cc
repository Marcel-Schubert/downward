...
LandmarkCountHeuristic::LandmarkCountHeuristic(const options::Options &opts)
    ...
    */
    ...
    lgraph = lm_graph_factory->compute_lm_graph(task, lm_status_manager_type == LAMA);
    // landmarks::dump_landmark_graph(task_proxy, *lgraph);
    lgraph->finalize();
    ...
    case MULTI_PATH:
        lm_status_manager =
            utils::make_unique_ptr<StatusManagerMultiPath>(
                *lgraph, add_goal_atoms, add_gn_parents);
        break;
    case LAMA:
        lm_status_manager =
            utils::make_unique_ptr<StatusManagerLama>(*lgraph);
        break;
    case FIXED_LAMA:
        lm_status_manager =
            utils::make_unique_ptr<StatusManagerFixedLama>(
                *lgraph, add_goal_atoms, add_gn_parents);
        break;
    case LM_ASTAR:
        lm_status_manager =
            utils::make_unique_ptr<StatusManagerLMAstar>(*lgraph);
        break;
    case ARO:
        lm_status_manager =
            utils::make_unique_ptr<StatusManagerARO>(*lgraph);
        break;
    }

    if (admissible) {
        if ((lm_status_manager_type == LAMA && reasonable_orders)
            || obedient_reasonable_orders) {
...
int LandmarkCountHeuristic::get_heuristic_value(const State &ancestor_state) {
    ...
    if (admissible) {
        ...
        for (int id = 0; id < lgraph->get_num_landmarks(); ++id) {
            if (lm_status_manager->landmark_is_future(id)) {
...
static shared_ptr<Heuristic> _parse(OptionParser &parser) {
    ...
    parser.add_option<bool>("add_goal_atoms", "add goal atoms in every step", "true");
    parser.add_option<bool>("add_gn_parents", "add greedy necessary parents in every step", "true");
    ...
    landmark_status_manager_types.push_back("FIXED_LAMA");
    landmark_status_manager_types.push_back("LM_ASTAR");
    landmark_status_manager_types.push_back("ARO");
    parser.add_enum_option<LandmarkStatusManagerType>(
            "status_manager",
            landmark_status_manager_types,
            "choose status manager type",
            "LAMA");
