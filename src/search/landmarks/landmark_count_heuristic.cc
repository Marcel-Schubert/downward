...
LandmarkCountHeuristic::LandmarkCountHeuristic(const options::Options &opts)
    : Heuristic(opts),
      ...
      conditional_effects_supported(opts.get<shared_ptr<LandmarkFactory>>(
    ...
    */
    ...
    case LAMA:
        ...
        break;
    case MULTI_PATH:
        // TODO: initialize with correct booleans.
        lm_status_manager =
            utils::make_unique_ptr<StatusManagerMultiPath>(
                *lgraph, false, false, false);
        break;
    case CONSISTENT:
        cerr << "CONSISTENT landmark status manager not allowed for the "
               "landmark count heuristic (at least not for now)." << endl;
        assert(false);
        utils::exit_with(ExitCode::SEARCH_INPUT_ERROR);
    }
...
int LandmarkCountHeuristic::get_heuristic_value(const State &ancestor_state) {
    ...
    if (admissible) {
        ...
        for (int id = 0; id < lgraph->get_num_landmarks(); ++id) {
            ...
            if (status == REQUIRED || status == ACCEPTED_AND_REQUIRED) {
