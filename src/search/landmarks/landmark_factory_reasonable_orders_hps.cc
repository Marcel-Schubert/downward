    ...
    for (auto &node_p : lm_graph->get_nodes()) {
        ...
        if (landmark.is_true_in_goal) {
            ...
            for (const auto &child : node_p->children) {
                ...
                if (edge2
                    >= EdgeType::GREEDY_NECESSARY) { // found node2_p: node_p ->_gn node2_p
...
bool LandmarkFactoryReasonableOrdersHPS::interferes(
    ...
    for (const FactPair &lm_fact_b : landmark_b.facts) {
        ...
        for (const FactPair &lm_fact_a : landmark_a.facts) {
            FactProxy fact_a = variables[lm_fact_a.var].get_fact(
                lm_fact_a.value);
            ...
            for (int op_or_axiom_id : op_or_axiom_ids) {
                ...
                EffectsProxy effects = get_operator_or_axiom(task_proxy,
                                                             op_or_axiom_id).get_effects();
            ...
            for (const pair<const int, int> &eff : shared_eff) {
                const FactProxy &effect_fact = variables[eff.first].get_fact(
                    eff.second);
...
bool LandmarkFactoryReasonableOrdersHPS::effect_always_happens(
    ...
    for (EffectProxy effect : effects) {
        ...
        if (effect_vars.find(var_id) != effect_vars.end()) {
            ...
            assert(effect_conditions_by_variable.find(var_id)
                   != effect_conditions_by_variable.end());
            int old_eff = effect_conditions_by_variable.find(
                var_id)->second.first;
            if (old_eff != value) {
        ...
        if (effect_conditions_by_variable.find(var_id)
            != effect_conditions_by_variable.end()
            && effect_conditions_by_variable.find(var_id)->second.first
            == value) {
            // We have seen this effect before, adding conditions
            for (FactProxy effect_condition : effect_conditions) {
                vector<FactPair> &vec = effect_conditions_by_variable.find(
                    var_id)->second.second;
    ...
    for (const auto &effect_conditions : effect_conditions_by_variable) {
        if (nogood_effect_vars.find(effect_conditions.first)
        ...
        for (const FactPair &cond : effect_conditions.second.second) {
            if (unique_conds.find(cond.var) != unique_conds.end()) {
                ...
                set<int> &the_set = unique_conds.emplace(cond.var,
